...
`benchmark.py` demo apps at the project root, or a future separate "Editing"
tool - without depending on anything outside `engine/`.
...
volumetric light) is a 0-3 level rather than a plain on/off flag, tunable
live from the in-engine settings panel (settings_ui.py, TAB to open).
"""
